...
void	ft_lstadd_elem(t_list **lst, t_list *new)
{
	t_list	*begin;
...
int	read_line(t_list *file_struct)
{
	...
	if (file_struct->content && ft_strchr(file_struct->content, '\n'))
		return (1);
	...
	while (result)
	{
		...
		free(tmp);
		if (ft_strchr(file_struct->content, '\n'))
			break ;
	}
	free(buf);
	return (result);
...
int	get_next_line(int fd, char **line)
{
	...
	if (!line || BUFFER_SIZE <= 0 || fd < 0 || read(fd, 0, 0) == -1)
		return (-1);
	if (!file_struct)
		ft_lstadd_elem(&file_struct, ft_lst_new_elem(fd));
	tmp = file_struct;
	while (tmp && tmp->fd != fd)
		tmp = tmp->next;
	if (!tmp)
	...
	del = tmp->content;
	tmp->content = ft_strdup(ft_strchr(del, '\n') + 1);
	free(del);
	return (1);
}
